Create a single horizontal sprite strip for the Codex app digital pet `gibbon` in the state `jumping`.
...
Output exactly 5 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Cream plush long-armed gibbon based on the provided reference image. Preserve the round face, cream fur, long arms, gentle smile, and overall silhouette as reference, but follow the hatch-pet-png skill output format..
